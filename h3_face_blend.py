...
class H3FaceBlend(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            description="修脸第 3 步: 缩放+像素贴回 (无VAE)。canvas 接②输出; 接①crop_images 则为恒等验收。",
            inputs=[
                io.Image.Input("images", tooltip="原视频画面 (主采样 latent 经 VAE Decode 后接这里)"),
                io.Image.Input("canvas", tooltip="待贴内容: ② 的 images; 干跑时接 ① 的 crop_images"),
                io.Dict.Input("bbox", tooltip="来自 H3FaceResample 的 bbox (或直连 H3FaceCut 的 face_pack)"),
                io.Int.Input("feather_px", default=16, min=0, max=128,
                             tooltip="贴回边缘羽化 (像素), 0=硬边; 接缝可见就加大"),
            ],
            outputs=[
    ...
    def execute(cls, images, canvas, bbox, feather_px=16) -> io.NodeOutput:
        ...
        if (h_can, w_can) != (S, S):
            ...
            print(f"[H3-FaceBlend] canvas {h_can}x{w_can} → {S}x{S} (lanczos, 本节点完成)")
        ...
        print(f"[H3-FaceBlend] 像素贴回 {F_img} 帧, 窗口 {S}x{S}, feather={f_px}px, 零VAE")
