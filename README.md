...
| `cut_detection` | `true` | **镜头检测闸门**：开启后自动检测真实镜头切换（使用 PySceneDetect），修正仅在同一镜头内部进行，真实切镜处直接跳过，避免色彩污染；关闭则回退到处理全部检测到的分段边界（可能对真实切镜造成误修）。 |
| `cut_threshold` | `15.0` | PySceneDetect 的灵敏度阈值（范围 `5.0` ~ `50.0`），值越小越敏感，推荐 `10` ~ `20`。仅当 `cut_detection=true` 时生效。 |
